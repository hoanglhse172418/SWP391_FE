...
function BookingPage() {
    ...
    const fetchVaccinationDetails = async () => {
      ...
      try {
        const profileRes = await api.get(
          `/VaccinationProfile/get-all?FilterOn=childrenId&FilterQuery=${selectedChild}`
        );
        ...
        const res = await api.get(
          `/VaccinationDetail/get-all?FilterOn=vaccinationProfileId&FilterQuery=${profileId}&PageSize=200`
        );
  ...
  useEffect(() => {
    ...
    if (remaining > 0) {
      setDiseaseInjectionInfo(
        `🦠 Bệnh "${selectedDisease}" còn ${remaining} mũi cần tiêm để hoàn tất.`
      );
    } else {
      setDiseaseInjectionInfo(
        `✅ Bệnh "${selectedDisease}" đã hoàn tất các mũi tiêm.`
      );
    }
    ...
    const fetchVaccineTemplateByChild = async () => {
      ...
      try {
        ...
        if (profiles.length > 0) {
          const profileId = profiles[0].id;
          const vaccineTemplateRes = await api.get(
            `/VaccineTemplate/get-by-profileid/${profileId}`
          );
          const vaccineData =
            vaccineTemplateRes.data?.$values || vaccineTemplateRes.data;
          ...
          vaccineData.forEach((vaccine) => {
            ...
            if (
              !diseaseToEarliestDate[diseaseId] ||
              date < new Date(diseaseToEarliestDate[diseaseId])
            ) {
          ...
          console.log(
            "🧠 Ngày tiêm dự kiến sớm nhất theo bệnh:",
            diseaseToEarliestDate
          );
        }
      } catch (error) {
    ...
    if (location.state) {
      ...
      if (location.state.expectedInjectionDate) {
        try {
          ...
          if (!isNaN(dateObj.getTime())) {
            ...
            console.warn(
              "expectedInjectionDate không hợp lệ:",
              location.state.expectedInjectionDate
            );
          }
        } catch (error) {
      ...
      if (location.state.diseaseId && diseases.length > 0) {
        const foundDisease = diseases.find(
          (d) => d.id === location.state.diseaseId
        );
        if (foundDisease) {
  ...
  useEffect(() => {
    if (token) {
      ...
      api
        .get(`/Child/get-all?FilterOn=userId&FilterQuery=${userId}`, {
  ...
  useEffect(() => {
    if (selectedChild) {
      api
        .get(`/Child/get-by-id/${selectedChild}`)
        .then((response) => {
        ...
        .catch((error) =>
          console.error("Lỗi khi lấy thông tin chi tiết của trẻ:", error)
        );
    }
  }, [selectedChild]);
  ...
  useEffect(() => {
    api
      .get("/VaccinePackage/get-all")
      .then((response) => {
        setVaccinePackages(response.data?.$values || []);
      })
      .catch((error) =>
        console.error("Lỗi khi lấy danh sách vaccine package:", error)
      );
  }, []);
  ...
  useEffect(() => {
    if (selectedDisease) {
      api
        .get(`/Vaccine/get-vaccines-by-diasease-name/${selectedDisease}`)
        .then((response) => {
  ...
  //       ...(res.data?.packageVaccineAppointments?.$values || []).flatMap(
  //         (pkg) => pkg.vaccineItems?.$values || []
  //       ),
  //     ];

  //     console.log("🧪 Tổng lịch tiêm của trẻ:", allAppointments.map(app => ({
  //       id: app.id,
  //       date: app.dateInjection,
  //       status: app.status,
  //       child: app.childrenId
  //     })));

  //     // Chuyển ngày về yyyy-mm-dd để so sánh
  //     const formattedDate = new Date(date).toISOString().split("T")[0];

  //     console.log(">>> Kiểm tra lịch trùng:");
  //     console.log("childId:", childId);
  //     console.log("Ngày kiểm tra:", formattedDate);
  //     console.log("Danh sách mũi tiêm:", allAppointments);
  ...
  const checkIfChildHasAppointmentOnDate = async (childId, date, excludeId = null) => {
    try {
      const res = await api.get(`/Appointment/customer-appointments`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  
      const singleAppointments = res.data?.singleVaccineAppointments?.$values || [];
      const packageAppointments = res.data?.packageVaccineAppointments?.$values || [];
  
      const packageVaccineItems = packageAppointments.flatMap(pkg =>
        (pkg.vaccineItems?.$values || []).map(item => ({
          ...item,
          childrenId: pkg.childrenId
        }))
      );
  
      const allAppointments = [...singleAppointments, ...packageVaccineItems];
      const formattedDate = new Date(date).toISOString().split("T")[0];
  
      const found = allAppointments.find(app => {
        const appDate = new Date(app.dateInjection).toISOString().split("T")[0];
        return (
          app.childrenId === parseInt(childId) &&
          appDate === formattedDate &&
          ["Pending", "Completed", "Waiting"].includes(app.status) &&
          app.id !== parseInt(excludeId) // ✅ bỏ qua chính mũi đang chỉnh sửa
        );
      });
  
      if (found) {
        console.log("❗ Trùng lịch:", found);
        return true;
      }
  ...
  const handleSubmit = async () => {
    if (vaccineType === "Vaccine lẻ" && selectedDisease) {
      ...
      if (minDate && new Date(appointmentDate) < new Date(minDate)) {
        openNotification(
          ...
          `⚠️ Ngày tiêm phải sau ${new Date(
            minDate
          ).toLocaleDateString()} theo lịch tiêm mẫu!`
        );
        return;
      }
    }
    ...
    // Check lịch trùng
    const isDuplicate = await checkIfChildHasAppointmentOnDate(
      selectedChild,
      appointmentDate
    );
    if (isDuplicate) {
      openNotification(
        "error",
        "Không thể đặt lịch",
        "Mỗi trẻ chỉ được tiêm 1 mũi/ngày!"
      );
      return;
    }
    ...
    if (vaccineType === "Vắc xin đang chờ" && selectedPendingVaccine) {
      try {
         // ✅ Tìm mũi đang chọn trong danh sách vaccine đang chờ
    const selectedVaccine = pendingVaccines.find(
      v => v.id === parseInt(selectedPendingVaccine)
    );
    if (!selectedVaccine) {
      openNotification('error', 'Không tìm thấy mũi tiêm đã chọn');
      return;
    }

    const oldDate = selectedVaccine.dateInjection;
    const oldDateObj = new Date(oldDate);
    const newDateObj = new Date(appointmentDate);

    // ✅ 1. So sánh ngày tiêm mới với ngày cũ
    if (newDateObj <= oldDateObj) {
      openNotification(
        'error',
        'Ngày không hợp lệ',
        `Ngày tiêm mới phải sau ngày đã lên lịch là ${oldDateObj.toLocaleDateString("vi-VN")}`
      );
      return;
    }

    // ✅ 2. Kiểm tra trùng lịch (ngoại trừ chính mũi đang chỉnh)
    const isDuplicate = await checkIfChildHasAppointmentOnDate(
      selectedChild,
      appointmentDate,
      selectedPendingVaccine // truyền thêm ID để loại trừ
    );
    ...
    if (isDuplicate) {
      ...
      return;
        ...
        openNotification(
          "error",
          "Lỗi cập nhật",
          `Cập nhật ngày tiêm thất bại! Lỗi: ${
    ...
    if (!vaccineTypeFormatted) {
      openNotification(
        "warning",
        "Sai loại vắc xin",
        "Vui lòng chọn loại vắc xin hợp lệ!"
      );
      return;
    }
    ...
    if (vaccineTypeFormatted === "Single") {
      const vaccine = relatedVaccines.find(
        (v) => v.id === parseInt(selectedVaccine)
      );
      if (vaccine?.inStockNumber === 0) {
        openNotification(
          "error",
          "Vắc xin đã hết hàng",
          "Không thể đặt lịch vì vắc xin đã hết!"
        );
        return;
      }
    }
    ...
    const requestData = {
      ...
      diaseaseName:
        vaccineTypeFormatted === "Single" ? selectedDisease || "" : "",
      selectedVaccineId:
        vaccineTypeFormatted === "Single"
          ? parseInt(selectedVaccine) || null
          : null,
      selectedVaccinePackageId:
        vaccineTypeFormatted === "Package"
          ? parseInt(selectedVaccinePackage) || null
          : null,
    ...
    try {
      if (vaccineTypeFormatted === "Package" && selectedVaccinePackage) {
        try {
          ...
          const selectedDateStr = new Date(appointmentDate)
            .toISOString()
            .split("T")[0];
          const hasOtherPackageSameDay = existingPackages.some((pkg) =>
            pkg.vaccineItems?.$values.some(
              (item) =>
                new Date(item.dateInjection).toISOString().split("T")[0] ===
                  selectedDateStr &&
      ...
      const fetchPendingVaccines = async () => {
        try {
          const response = await api.get(
            `/Appointment/get-appointments-from-buying-package/${selectedChild}`,
            {
              headers: {
          ...
          const pendingList =
            data.packageVaccineAppointments?.$values?.flatMap((pkg) =>
              pkg.vaccineItems?.$values.filter(
          ...
          <div className="row">
            <div className="col-6">
              <div className="BookingPage-flex">
                ...
                <div className="BookingPage-flex5">
                  <button
                    className={`Booking-goi ${
                      vaccineType === "Vắc xin gói" ? "selected" : ""
                    }`}
                    onClick={() => setVaccineType("Vắc xin gói")}
                  >
                    Vắc xin gói
                  </button>
                  <button
                    className={`Booking-goi ${
                      vaccineType === "Vaccine lẻ" ? "selected" : ""
                    }`}
                    onClick={() => setVaccineType("Vaccine lẻ")}
                  >
                    Vắc xin lẻ
                  </button>
                  <button
                    className={`Booking-goi ${
                      vaccineType === "Vắc xin đang chờ" ? "selected" : ""
                    }`}
                    onClick={() => setVaccineType("Vắc xin đang chờ")}
                  >
                    Vắc xin đang chờ
                  </button>
                </div>
                    ...
                    {showVaccineSelect && relatedVaccines.length > 0 && (
                      <>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setSelectedVaccine(Number(e.target.value))
                          }
                        >
                ...
                {vaccineType === "Vắc xin gói" && (
                  <>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setSelectedVaccinePackage(Number(e.target.value))
                      }
                    >
                ...
                {vaccineType === "Vắc xin đang chờ" &&
                  pendingVaccines.length > 0 && (
                    <>
                      <div className="BookingPage-tuade">
                        Danh sách vắc xin đang chờ
                      </div>
                      <select
                        className="BookingPage-input"
                        ...
                        onChange={(e) => {
                          ...
                          setAppointmentDate(
                            selectedVaccine?.dateInjection.split("T")[0] || ""
                          );
                        }}
                      >
                        <option value="">Chọn vắc xin</option>
                        {pendingVaccines.map((vaccine) => (
                          <option key={vaccine.id} value={vaccine.id}>
                            {vaccine.vaccineName} - Ngày tiêm:{" "}
                            {new Date(
                              vaccine.dateInjection
                            ).toLocaleDateString()}
                          </option>
                        ))}
                      </select>
                ...
                {/* NÚT HOÀN THÀNH */}
                <button className="BookingPage-button" onClick={handleSubmit}>
                  Hoàn thành đăng ký
                </button>
              </div>
            </div>
